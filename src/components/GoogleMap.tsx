...
const createMarkerContent = (url: string, number: number | null, width: number, height: number): HTMLElement => {
...
const GoogleMap = forwardRef<MapRef, NavigationMapProp>(
  (
    ...
    const addMarkerListener = useCallback((
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      marker: { addListener: (event: string, handler: (e: any) => void) => void; },
      content: string,
    ) => {
      marker.addListener('click', () => {
        if (infoWindowRef.current && mapInstance.current) {
          infoWindowRef.current.setContent(content);
          infoWindowRef.current.open(mapInstance.current, marker as google.maps.Marker);
        }
      });
    }, []);
    ...
    useEffect(() => {
      const initializeMap = async () => {
        ...
        try {
          ...
          const defaultCenter = { lat: 22.3165316829187, lng: 114.182081980287 };
          ...
          message.error("Google Maps API failed to load. Please check API key and your internet connection.");
        }
      };
      initializeMap();
    }, [message]);
    ...
    useEffect(() => {
      ...
      if (!map || typeof google.maps.marker.AdvancedMarkerElement === 'undefined') return;
