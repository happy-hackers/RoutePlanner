...
export const useRoutingAndGeocoding = ({
    ...
    const processRouteResult = async (
        ...
        const totalTime = optimizedRouteResult.total_time;
        const totalDistance = optimizedRouteResult.total_distance;
        const segmentTimes = optimizedRouteResult.segment_times;

        const startCoord = optimizedRouteResult.startCoord || preCalculatedStartCoord || await geocodeAddress(startAddress);
        const endCoord = optimizedRouteResult.endCoord || preCalculatedEndCoord || await geocodeAddress(endAddress);

        const sortedOrders = routeOrder.map(
        ...
        const coordinates = await fetchOsrmPolyline(startCoord, endCoord, orderedWaypoints);
    ...
    const calculateRoutebyTime = async (dispatcherId?: number) => {
        ...
        try {
            ...
            if (dispatcherId)
                oMarkers = [...orderMarkers.filter((m) => m.dispatcherId === dispatcherId)];
            ...
            if (!optimizedRouteResult.error) {
                await processRouteResult(optimizedRouteResult, dispatcherId, oMarkers, startCoord, endCoord);
            }
        } catch (error) {
